...
class TestBudgetPartitioner:
    ...
    def test_non_integer_budgets(self):
        ...
        budget = BudgetPartitioner(costs=costs, total_non_clifford_budget=10, max_partition_value=5)
    ...
    def test_integer_budgets_3(self):
        ...
        budget = BudgetPartitioner(costs=costs, total_non_clifford_budget=10, max_partition_value=6)
        partition = budget.partition()
        assert partition == [[0],
                             [1],
                             [2],
    ...
    def test_integer_budgets_2(self):
        ...
        budget = BudgetPartitioner(costs=costs, total_non_clifford_budget=10, max_partition_value=4)
        partition = budget.partition()
        assert partition == [[0],
                             [1], 
                             [2],
    ...
    def test_integets_2(self):
        costs = {"T": 1.0, "sqrtT": 3.0}
        budget = BudgetPartitioner(costs=costs, total_non_clifford_budget=6, max_partition_value=4)
        partition = budget.partition()
        print(partition)

    @pytest.mark.parametrize("max_partition_value", [
        1,2,3,4
    ])
    def test_max_partition_value_too_small(self, max_partition_value: int):
        costs = {"T": 1.0, "sqrtT": 2.5}
        with pytest.raises(AssertionError):
            budget = BudgetPartitioner(costs=costs, total_non_clifford_budget=10,
